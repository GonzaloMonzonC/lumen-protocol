...
def _pdb_save_all() -> None:
    ...
    try:
        ...
        for i, p_item in enumerate(_preserved):
            pairs.append(("STATE", f"global:preserved:{i}".encode(), json.dumps(p_item).encode()))
        for i, t_item in enumerate(_call_timeline):
            pairs.append(("STATE", f"global:timeline:{i}".encode(), json.dumps(t_item).encode()))
...
if insert_point > 0:
    ...
    print('✓ Functions inserted before def main()')
else:

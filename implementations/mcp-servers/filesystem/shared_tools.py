"""
Shared tool definitions and implementations for LUMEN Filesystem MCP servers.

Used by both server.py (JSON-RPC transport) and server_native.py (LUMEN binary
transport).  The two servers share identical tool logic; only the transport layer
differs.
"""
...
def _get_allowed_roots() -> list[str]:
    ...
    if _ALLOWED_ROOTS is None:
        env = os.environ.get("ALLOWED_ROOTS", "").strip()
        if env:
            _ALLOWED_ROOTS = [os.path.realpath(p) for p in env.split(",") if p.strip()]
        else:
            _ALLOWED_ROOTS = [os.path.realpath(os.getcwd())]
    return _ALLOWED_ROOTS

def resolve_path(path: str) -> Path:
    """Resolve a path that may start with ~/, with sandbox enforcement."""
    # Expand ~/
    if path.startswith("~/"):
        resolved = Path.home() / path[2:]
    elif path.startswith("~\\"):
        resolved = Path.home() / path[2:]
    else:
        resolved = Path(path)

    # Resolve to absolute, normalized path (follows symlinks)
    try:
        resolved = resolved.resolve()
    except Exception:
        resolved = resolved.absolute()

    # Sandbox check — ensure resolved path is within an allowed root

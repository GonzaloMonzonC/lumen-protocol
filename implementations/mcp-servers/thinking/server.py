...
def tool_niche_create(args: dict) -> dict:
    """Create a new cognitive niche (project/area)."""
    global _next_niche_id, _niches
    name = args.get("name", "").strip()
    desc = args.get("desc", "").strip()
    color = args.get("color", "#22d3ee").strip()
    columns = args.get("columns", ["Backlog", "In Progress", "Review", "Done", "Blocked"])
    if not name:
        return {"content": [{"type": "text", "text": "❌ Niche name required"}]}
    # Generate ID
    niche_id = f"niche_{_next_niche_id}"
    _next_niche_id += 1
    niche = {
        "id": niche_id,
        "name": name,
...
def tool_niche_list(args: dict) -> dict:
    """List all cognitive niches."""
    global _niches
    if not _niches:
        return {"content": [{"type": "text", "text": "No niches defined. Use niche_create to create one."}]}
    lines = [f"📋 Niches ({len(_niches)}):"]
    for nid, niche in _niches.items():
        lines.append(f"  • {niche['name']} (ID: {nid}) - {niche['desc'][:50]}{'...' if len(niche['desc']) > 50 else ''}")
        lines.append(f"    Columns: {', '.join(niche['columns'])}")
        lines.append(f"    Color: {niche['color']}")
    return {"content": [{"type": "text", "text": "\n".join(lines)}]}

def tool_niche_update(args: dict) -> dict:
    """Update niche properties (name, desc, color, columns, archived)."""
    global _niches
    niche_id = args.get("niche_id", "").strip()
    if not niche_id:
        return {"content": [{"type": "text", "text": "❌ niche_id required"}]}
    if niche_id not in _niches:
        return {"content": [{"type": "text", "text": f"❌ Niche not found: {niche_id}"}]}
    niche = _niches[niche_id]
    if "name" in args and args["name"] is not None:
        niche["name"] = args["name"].strip()
    if "desc" in args and args["desc"] is not None:
        niche["desc"] = args["desc"].strip()
    if "color" in args and args["color"] is not None:
        niche["color"] = args["color"].strip()
    if "columns" in args and args["columns"] is not None:
        niche["columns"] = args["columns"]
...
def tool_task_create(args: dict) -> dict:
    """Create a new task in a niche."""
    global _next_task_id, _tasks, _niches
    niche_id = args.get("niche_id", "").strip()
    title = args.get("title", "").strip()
    desc = args.get("desc", "").strip()
    ...
    task = {
        ...
        "title": title,
        "desc": desc,
        "status": _niches[niche_id]["columns"][0],  # default to first column (Backlog)
        "priority": priority,
        "tags": tags,
...
def tool_task_move(args: dict) -> dict:
    """Move a task to a column (or edit fields)."""
    global _tasks, _niches
    task_id = args.get("task_id", "").strip()
    to_column = args.get("to_column", "").strip()
    if not task_id:
        return {"content": [{"type": "text", "text": "❌ task_id required"}]}
    if task_id not in _tasks:
        return {"content": [{"text": "text", "text": f"❌ Task not found: {task_id}"}]}
    task = _tasks[task_id]
    niche = _niches.get(task["niche_id"])
    if not niche:
        return {"content": [{"type": "text", "text": f"❌ Niche not found for task: {task['niche_id']}"}]}
    # Update fields if provided
    if "title" in args and args["title"] is not None:
        task["title"] = args["title"].strip()
    if "desc" in args and args["desc"] is not None:
        task["desc"] = args["desc"].strip()
    ...
    if "assignee" in args and args["assignee"] is not None:
        task["assignee"] = args["assignee"].strip() or None
    # Move column
    if to_column:
        if to_column not in niche["columns"]:
            return {"content": [{"type": "text", "text": f"❌ Invalid column: {to_column}. Valid columns: {', '.join(niche['columns'])}"}]}
        task["status"] = to_column
        if to_column == _niches[task["niche_id"]]["columns"][-1]:  # assuming last column is Done
...
def tool_task_link(args: dict) -> dict:
    """Link a task to LUMEN cognitive objects (chain, pattern, decision, wiki)."""
    global _tasks
    task_id = args.get("task_id", "").strip()
    if not task_id:
        return {"content": [{"type": "text", "text": "❌ task_id required"}]}
    ...
    if "chain_id" in args and args["chain_id"]:
        ...
        if chain_id not in task["references"]["chains"]:
            task["references"]["chains"].append(chain_id)
            linked = True
    if "pattern_id" in args and args["pattern_id"]:
        pattern_id = args["pattern_id"].strip()
        if pattern_id not in task["references"]["patterns"]:
            task["references"]["patterns"].append(pattern_id)
            linked = True
    if "decision_id" in args and args["decision_id"]:
        decision_id = args["decision_id"].strip()
        if decision_id not in task["references"]["decisions"]:
            task["references"]["decisions"].append(decision_id)
            linked = True
    if "wiki_id" in args and args["wiki_id"]:
        wiki_id = args["wiki_id"].strip()
        if wiki_id not in task["references"]["wikis"]:
            task["references"]["wikis"].append(wiki_id)
            linked = True
    if linked:
        task["updated_at"] = time.time()
        return {"content": [{"type": "text", "text": f"✅ Task linked to cognitive objects"}]}
    else:
        return {"content": [{"type": "text", "text": "⚠️ No valid links provided"}]}
...
def tool_task_list(args: dict) -> dict:
    ...
    for task in _tasks.values():
        ...
        if tag and tag not in task["tags"]:
            continue
        if search:
            searchable = f"{task['title']} {task['desc']} {' '.join(task['tags'])}".lower()
    ...
    lines = [f"📋 Tasks ({len(filtered)} shown, {len(_tasks)} total):"]
    for task in filtered:
        niche = _niches.get(task["niche_id"])
        niche_name = niche["name"] if niche else "unknown"
        status_icon = {"Backlog": "📥", "In Progress": "🔧", "Review": "👀", "Done": "✅", "Blocked": "🚫"}.get(task["status"], "📎")
        priority_icon = {"low": "▼", "medium": "◇", "high": "▲", "critical": "!!"}.get(task["priority"], "")
        refs = []
        if task["references"]["chains"]:
            refs.append(f"🔗{len(task['references']['chains'])}")
        if task["references"]["patterns"]:
            refs.append(f"🐞{len(task['references']['patterns'])}")
        if task["references"]["decisions"]:
            refs.append(f"📋{len(task['references']['decisions'])}")
        if task["references"]["wikis"]:
            refs.append(f"📖{len(task['references']['wikis'])}")
        refs_str = " " + " ".join(refs) if refs else ""
        lines.append(f"  {status_icon} {priority_icon} #{task['id']} [{niche_name}] {task['title']}{refs_str}")
        if task["desc"]:
            lines.append(f"      {task['desc'][:100]}{'...' if len(task['desc']) > 100 else ''}")
    return {"content": [{"type": "text", "text": "\n".join(lines)}]}


def tool_task_delete(args: dict) -> dict:
...
def tool_kanban_stats(args: dict) -> dict:
    """Show kanban statistics per niche."""
    global _niches, _tasks
    niche_id = args.get("niche_id", "").strip()
    
    if niche_id and niche_id not in _niches:
        return {"content": [{"type": "text", "text": f"❌ Niche not found: {niche_id}"}]}
    
    niches_to_show = [niche_id] if niche_id else list(_niches.keys())
    
    lines = ["📊 Kanban Statistics"]
    if niche_id:
        lines.append(f"   (filtered: {_niches[niche_id]['name']})")
    lines.append("")
    ...
    for nid in niches_to_show:
        ...
        lines.append(f"📁 {niche['name']} ({nid})")
        lines.append(f"   Total tasks: {len(niche_tasks)}")
        
        # Count by column
        col_counts = {}
        for t in niche_tasks:
            col_counts[t["status"]] = col_counts.get(t["status"], 0) + 1
        lines.append(f"   By column: " + " · ".join(f"{c}: {n}" for c, n in col_counts.items()))
        ...
        if prio_counts:
            lines.append(f"   By priority: " + " · ".join(f"{p}: {n}" for p, n in prio_counts.items()))
        ...
        lines.append(f"   Total links to cognitive objects: {links_count}")
        
        # Blockers count
        blockers_count = sum(1 for t in niche_tasks if t.get("blockers"))
        if blockers_count:
            lines.append(f"   Blocked tasks: {blockers_count}")
        
        # Avg time in each column? We'd need timestamps, skip for now.
        lines.append("")
    
    return {"content": [{"type": "text", "text": "\n".join(lines)}]}


def tool_task_search(args: dict) -> dict:
    """Search tasks across niches by title, description, tags, and references."""
    global _tasks, _niches
    ...
    if not query and not status and not priority and not tag and not niche_id:
        return {"content": [{"type": "text", "text": "❌ At least one search filter required"}]}
    ...
    for tid, task in _tasks.items():
        ...
        if priority and task["priority"] != priority:
            continue
        # Tag filter
        if tag and tag not in [t.lower() for t in task.get("tags", [])]:
            continue
        # Text search
        if query:
            searchable = f"{task['title']} {task.get('desc','')} {' '.join(task.get('tags',[]))} {tid}".lower()
            searchable += " " + " ".join(task.get("references", {}).get("chains", []))
            searchable += " " + " ".join(task.get("references", {}).get("patterns", []))
            if query not in searchable:
                continue
        results.append(task)
    
    if not results:
        return {"content": [{"type": "text", "text": "🔍 No tasks found matching your search."}]}
    ...
    for t in results:
        niche = _niches.get(t["niche_id"], {})
        niche_name = niche.get("name", t["niche_id"])
        prio_icon = {"low":"▽","medium":"◇","high":"▲","critical":"!!"}.get(t.get("priority","medium"),"◇")
        refs = []
        if t.get("references",{}).get("chains"): refs.append(f"🔗{len(t['references']['chains'])}")
        if t.get("references",{}).get("patterns"): refs.append(f"🐞{len(t['references']['patterns'])}")
        if t.get("references",{}).get("decisions"): refs.append(f"📋{len(t['references']['decisions'])}")
        ref_str = " " + " ".join(refs) if refs else ""
        lines.append(f"  {prio_icon} #{t['id']} [{niche_name}] {t['title']}{ref_str}")
        if t.get("desc"):
            desc = t["desc"][:80] + "..." if len(t["desc"]) > 80 else t["desc"]
            lines.append(f"      {desc}")
        lines.append(f"      Status: {t['status']} · Priority: {t.get('priority','medium')}" + 
...
HANDLERS = {
    ...
    "niche_create": tool_niche_create,
    "niche_list": tool_niche_list,
